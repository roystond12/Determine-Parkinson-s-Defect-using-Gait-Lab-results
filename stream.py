...
stream = Blueprint("stream",__name__,static_folder="static",template_folder="templates")
with open('parkinson_model.pkl', 'rb') as file:
    model = pickle.load(file)

@stream.route('/livestream', methods=['GET'])
def livestream():
    ...
    while True:
        ...
        if list:
            step_mean = get_step_length_mean(list) or 0
            velocity = get_velocity([list]) or 0
            cadence = get_cadence([list[27][1]]) or 0
            amb_time = get_ambulation_time(len([list])) or 0
            left_egvi = get_egvi_variance([list]) or 0
            right_egvi = get_egvi_variance([swap_foot_indices(list)]) or 0
            mean_egvi = get_mean_egvi(left_egvi, right_egvi) or 0
